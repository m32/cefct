...
int main(int argc, char** argv) {
    ...
    printf("cef_settings=%ld\n", settings.size);
    settings.log_severity = LOGSEVERITY_WARNING; // Show only warnings/errors
    settings.no_sandbox = 1;
#if 1
    {
    char bsp[] = "/devel/00mirror-cvs/00-m32/cefct/bin/cefclient";
    cef_string_utf8_to_utf16(bsp, strlen(bsp), &settings.browser_subprocess_path);
    }
    {
    char bsp[] = "/devel/00mirror-cvs/00-m32/cefct/bin-cefdata/root";
    cef_string_utf8_to_utf16(bsp, strlen(bsp), &settings.root_cache_path);
    }
    {
    char bsp[] = "/devel/00mirror-cvs/00-m32/cefct/bin-cefdata/root/cache";
    cef_string_utf8_to_utf16(bsp, strlen(bsp), &settings.cache_path);
    }
    {
    char bsp[] = "/devel/00mirror-cvs/00-m32/cefct/bin-cefdata/user-data";
    cef_string_utf8_to_utf16(bsp, strlen(bsp), &settings.user_data_path);
    }
    {
    char bsp[] = "/devel/00mirror-cvs/00-m32/cefct/bin-cefdata/cef.log";
    cef_string_utf8_to_utf16(bsp, strlen(bsp), &settings.log_file);
    }
#endif
